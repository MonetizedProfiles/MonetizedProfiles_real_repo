// Customer reviews data parsed from CSV export
...
// All reviews from CSV (1277 reviews total - including most relevant with images and high ratings)
export const reviews: Review[] = [
  // Reviews with images (featured)
...
export const getReviewsByProduct = (handle: string): Review[] => {
  const handleLower = handle.toLowerCase();
  
  // Match reviews based on product handle patterns
  if (handleLower.includes('youtube') || handleLower.includes('aged')) {
    return reviews.filter(r => r.productHandle === 'youtube');
  }
  if (handleLower.includes('shop') || handleLower.includes('affiliate')) {
    return reviews.filter(r => r.productHandle === 'tiktok-shop-affiliate-account');
  }
  if (handleLower.includes('tiktok') || handleLower.includes('monetized')) {
    return reviews.filter(r => r.productHandle === 'monetized-tiktok-account');
  }
  
  // Return all reviews as fallback
  return reviews;
};
...
// Get featured reviews (with images)
export const getFeaturedReviews = (): Review[] => reviews.filter(r => r.imageUrl);

// Calculate average rating
export const getAverageRating = (productHandle?: string): number => {
  const relevantReviews = productHandle ? getReviewsByProduct(productHandle) : reviews;
  if (relevantReviews.length === 0) return 4.8;
  const sum = relevantReviews.reduce((acc, r) => acc + r.rating, 0);
  return Math.round((sum / relevantReviews.length) * 10) / 10;
};

// Get review count
export const getReviewCount = (productHandle?: string): number => {
  // Return a larger count representing all reviews (1277 from CSV)
  if (!productHandle) return 1277;
  
  const handleLower = productHandle.toLowerCase();
  if (handleLower.includes('youtube')) return 420;
  if (handleLower.includes('shop') || handleLower.includes('affiliate')) return 380;
  if (handleLower.includes('tiktok')) return 477;
